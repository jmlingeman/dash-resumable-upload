...
_components = _dash.development.component_loader.load_components(
    _os.path.join(_current_path, "metadata.json"), "dash_resumable_upload"
)
...
def decorate_server(server, temp_base, s3_interface):
    ...
    def resumable():
        ...
        resumableChunkNumber = _request.args.get("resumableChunkNumber", type=int)

        if not (resumableIdentfier and resumableFilename and resumableChunkNumber):
    ...
    def resumable_post():
        resumableTotalChunks = _request.form.get("resumableTotalChunks", type=int)
        resumableChunkNumber = _request.form.get(
            "resumableChunkNumber", default=1, type=int
        )
        resumableFilename = _request.form.get(
            "resumableFilename", default="error", type=str
        )
        resumableIdentfier = _request.form.get(
            "resumableIdentifier", default="error", type=str
        )
        resumableRelativePath = _request.form.get(
            "resumableRelativePath", default="", type=str
        )
        subject_name = _request.form.get("subject", default="", type=str)
        ...
        if upload_complete:
            ...
            while any(
                [
                    _os.path.isfile(_os.path.join(temp_dir, ".lock_{:d}".format(chunk)))
                    for chunk in range(1, resumableTotalChunks + 1)
                ]
            ):
                _time.sleep(1)
            # Make sure some other chunk didn't trigger file reconstruction
            target_file_name = _os.path.join(
                temp_base, resumableRelativePath, resumableFilename
            )
            target_folder_name = _os.path.join(temp_base, resumableRelativePath)
            print(target_file_name, target_folder_name)
            _os.makedirs(target_folder_name, exist_ok=True)
